            ...
            <div className="max-w-3xl mx-auto">
                ...
                <div ref={headerRef} className="flex flex-col items-center justify-center text-center mb-24">
                    <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-white/10 bg-white/5 backdrop-blur-sm mb-8">
                        ...
                        <span className="text-[10px] font-bold text-gray-300 tracking-[0.2em] uppercase">FAQs</span>
                    </div>
                    <h2 className="text-5xl md:text-7xl font-sans tracking-tight">FAQs</h2>
                </div>
                    ...
                    {faqs.map((faq, index) => (
                        <div key={index} className="border-b border-white/10 last:border-none">
                            ...
                            >
                                ...
                                <div className={`w-10 h-10 rounded-full flex items-center justify-center border transition-all duration-300 ${openIndex === index ? 'bg-white border-white text-black' : 'bg-transparent border-white/20 text-gray-400 group-hover:border-white group-hover:text-white'}`}>
                                    {openIndex === index ? <Minus size={18} /> : <Plus size={18} />}
                                </div>
                            </button>

                            <div
                                className={`grid transition-all duration-500 ease-in-out ${openIndex === index ? 'grid-rows-[1fr] opacity-100 mb-8' : 'grid-rows-[0fr] opacity-0'}`}
                            >
